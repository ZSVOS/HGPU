...
def save_checkpoint_epoch(args, encoder, decoder, enc_opt, dec_opt, epoch, best=False):
    ...
    torch.save(encoder.state_dict(), os.path.join('model', args.model_name, 'encoder_{}.pt'.format(epoch)))
    torch.save(decoder.state_dict(), os.path.join('model', args.model_name, 'decoder_{}.pt'.format(epoch)))
    torch.save(enc_opt.state_dict(), os.path.join('model', args.model_name, 'enc_opt_{}.pt'.format(epoch)))
    torch.save(dec_opt.state_dict(), os.path.join('model', args.model_name, 'dec_opt_{}.pt'.format(epoch)))
...
def load_checkpoint_epoch(model_name, epoch, use_gpu=True, load_opt=True):
    if use_gpu:
        encoder_dict = torch.load(os.path.join('model', model_name, 'encoder_{}.pt'.format(epoch)))
        decoder_dict = torch.load(os.path.join('model', model_name, 'decoder_{}.pt'.format(epoch)))
        if load_opt:
            enc_opt_dict = torch.load(os.path.join('model', model_name, 'enc_opt_{}.pt'.format(epoch)))
            dec_opt_dict = torch.load(os.path.join('model', model_name, 'dec_opt_{}.pt'.format(epoch)))
    else:
        encoder_dict = torch.load(os.path.join('model', model_name, 'encoder_{}.pt'.format(epoch)),
